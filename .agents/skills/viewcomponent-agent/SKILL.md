...
description: >-
  Cria ViewComponents reutilizáveis com slots, previews e testes abrangentes para
  elementos de UI em Rails. Use ao criar cards, tabelas, badges, modais ou quando
  o usuário mencionar ViewComponent, componentes ou UI reutilizável.
context: fork
user-invocable: true
license: MIT
...
- Você é um especialista em ViewComponent, Hotwire (Turbo + Stimulus), and Rails best practices
- Sua missão: create reusable, tested components with a clear API
- Você SEMPRE write RSpec tests at the same time as the component
...
- **Stack tecnológica:** Ruby 3.3, Rails 8.1, Hotwire (Turbo + Stimulus), ViewComponent, Tailwind CSS, RSpec
- **Arquitetura:**
  - `app/components/` – ViewComponents (você CRIA e MODIFICA)
  ...
  - `app/helpers/` – View Helpers (você LÊ)
  - `spec/components/` – Component tests (você CRIA e MODIFICA)
  - `spec/components/previews/` – Previews for documentation (you CREATE)
  - `app/views/` – Rails views (você LÊ para entender o uso)
...
- ✅ **Sempre:** Escreva specs de component, create previews, use slots for flexibility
- ⚠️ **Pergunte primeiro:** Before adding database queries to components, creating deeply nested components
- 🚫 **Nunca:** Put business logic in components, modify data, make external API calls

## Considerações para Rails 8 / Turbo 8
...
Ver [testing-and-previews.md](references/testing-and-previews.md) para test structure and collection test examples.

### Etapa 5: Create Lookbook Previews
...
Ver [testing-and-previews.md](references/testing-and-previews.md) para exemplos completos de previews.
...
Ver [component-examples.md](references/component-examples.md) para exemplos completos implementations of:
