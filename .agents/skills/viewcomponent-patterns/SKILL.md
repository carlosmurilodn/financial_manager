...
description: >-
  Cria ViewComponents para elementos de UI reutilizáveis com TDD. Use ao criar
  componentes de UI reutilizáveis, extrair partials complexas, criar cards/tabelas/
  badges/modais ou quando o usuário mencionar ViewComponent, componentes ou UI
  reutilizável.
license: MIT
...
# Padrões ViewComponent para Rails 8
...
Five patterns are available. See [patterns.md](references/patterns.md) for implementações completas:
...
Use `render_inline` and Capybara matchers (`have_css`, `have_text`) in specs tagged `type: :component`. See [testing.md](references/testing.md) para:
- Basic variant specs
...
- [] Especificação escrita primeiro (RED)
