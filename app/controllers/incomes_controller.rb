class IncomesController < ApplicationController
  ...
  def index
    ...
    @filter_date = if @month.present? && @year.present?
                     Date.new(@year, @month, 1) rescue Date.today
                   else
                     Date.today
                   end

    @incomes = Income.where(balance_month: @filter_date.beginning_of_month..@filter_date.end_of_month).order(:date)
  ...
  def total_expenses_until(date)
    # Soma das despesas normais (não parceladas) pagas até a data
    normal_expenses = Expense.where("balance_month <= ? AND paid = true", date)
                            .where(paid: true)
                            .sum(:amount)

    # Soma das parcelas pagas até a data
    installments_paid = Installment.where("due_date <= ? AND paid = true", date).sum(:amount)

    # Retorna o total das duas somas
